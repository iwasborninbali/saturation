...
case "$1" in
  start)   # start M S_from S_to [sweeps]
    ...
    ;;
  status)
    gcloud compute ssh $VM --zone $Z --quiet --command "cd ~/anneal 2>/dev/null && ls *_T0.txt 2>/dev/null; for f in anneal_n7_m*_s*.log; do echo \"\$f: \$(tail -1 \$f | cut -c1-110)\"; done; uptime"
    ;;
  fetch)
