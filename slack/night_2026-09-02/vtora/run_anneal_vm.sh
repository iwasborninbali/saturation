...
case "$1" in
  start)   # start M S_from S_to [sweeps]
    M=${2:-74}; S1=${3:-1}; S2=${4:-8}; SW=${5:-20000}
    gcloud compute scp --zone $Z --quiet $HERE/cube_anneal.py $VM:~/cube_anneal.py
    gcloud compute ssh $VM --zone $Z --quiet --command "mkdir -p ~/anneal && cd ~/anneal && for s in \$(seq $S1 $S2); do nohup python3 ~/cube_anneal.py 7 $M $SW 200 \$s anneal_n7_m${M}_s\$s > anneal_n7_m${M}_s\$s.log 2>&1 & done; sleep 2; ps aux | grep -c '[c]ube_anneal'"
    ;;
  status)
    gcloud compute ssh $VM --zone $Z --quiet --command "cd ~/anneal 2>/dev/null && ls *_T0.txt 2>/dev/null; for f in anneal_n*_m*_s*.log; do echo \"\$f: \$(tail -1 \$f | cut -c1-110)\"; done; uptime"
    ;;
  fetch)
