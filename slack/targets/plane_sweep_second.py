...
# ПЕРЕМЕШИВАНИЕ ОСТАТКА. Доли берут каждый nsh-й индекс, что выглядит как равномерная выборка,
# но ВНУТРИ доли обход идёт по возрастанию. После возобновления каждый работник встаёт на свой
# ПЕРВЫЙ несделанный индекс, а они соседние — и все тридцать оказываются в одной узкой полосе.
# Трудность с индексом связана (кучные тройки умирают мгновенно, разбросанные сопротивляются),
# поэтому вся бригада попадает в одну область и стоит. Замерено: у напарника 1.3 закрытия
# на работника за 10 минут, у меня 0.033 — сорок раз разницы при одинаковых решателях.
# Перемешивание детерминировано (семя от номера доли), поэтому возобновление воспроизводимо.
import random as _r
_r.Random(1000 + shard).shuffle(mine)
