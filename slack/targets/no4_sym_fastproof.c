...
static unsigned char *dseen; static int DM,DSPAN; static int dtmp[128], dtn;
...
static void dir_mark(int from, int to, int val){   /* пометить направления пар (i<from) x (from..to-1) и внутри хвоста */
    for(int i=from;i<to;i++){
        for(int j=0;j<i;j++) dseen[dircode(pts[i].x-pts[j].x,pts[i].y-pts[j].y,pts[i].z-pts[j].z)]=val;
    }
}
/* совместима ли орбита o с текущим набором */
static int ok_add(int o){
    int k=orbsz[o], tot=npts+k; P all[64];
    dtn=0;                                  /* дешёвый необходимый фильтр перед полной проверкой */
    for(int i=0;i<k;i++){
        for(int j=0;j<npts;j++){ int c=dircode(orb[o][i].x-pts[j].x,orb[o][i].y-pts[j].y,orb[o][i].z-pts[j].z);
            if(dseen[c]){ for(int t=0;t<dtn;t++) dseen[dtmp[t]]=0; return 0; } dseen[c]=1; dtmp[dtn++]=c; }
        for(int j=0;j<i;j++){ int c=dircode(orb[o][i].x-orb[o][j].x,orb[o][i].y-orb[o][j].y,orb[o][i].z-orb[o][j].z);
            if(dseen[c]){ for(int t=0;t<dtn;t++) dseen[dtmp[t]]=0; return 0; } dseen[c]=1; dtmp[dtn++]=c; }
    }
...
static void dfs(int *live,int nlive){
    ...
    for(int i=0;i<nlive;i++){
        ...
        if(depth0==1 && shard2>=0 && (i%nshard2)!=shard2) continue;
