/* witness3d.c — поиск больших конфигураций без четырёх компланарных в кубе [n]^3.
 *
 * ЗАЧЕМ. У A280537 данные обрываются на a(8)=20; девятого члена нет ни у кого. Энтропийный
 * порог предсказывает a(9) ~ 22, и предсказание проверяется тем, что 22 точки при n=9 либо
 * находятся, либо нет. Найденная конфигурация даёт ПЕРВУЮ нижнюю границу a(9).
 *
 * УСТРОЙСТВО — ПО СМЕРТИ. Клетка жива, пока через неё не прошла ни одна мёртвая прямая
 * (две точки) и ни одна мёртвая плоскость (три точки). Растим от пустоты, пока живые есть.
 * Конфигурация максимальна не достижением, а исчерпанием.
 *
 * ПОЧЕМУ ПЕРЕЗАПУСКИ, А НЕ УЛУЧШЕНИЕ. Измерено: у конфигурации вблизи максимума СОСЕДЕЙ НЕТ —
 * удаление любой точки оживляет ровно одну клетку, ту же самую. Локальный поиск двигаться
 * не может в принципе. Значит растить от пустоты и перезапускаться.
 *
 * ГЛАВНАЯ ПОПРАВКА ПРОТИВ ПЕРВОЙ РЕДАКЦИИ: маска плоскости снимается ТОЛЬКО С ЖИВЫХ клеток,
 * а не со всего куба. Живых к концу десятки, клеток в кубе сотни — первая редакция считала
 * в двадцать раз лишнего и потому не успевала сделать нужное число перезапусков.
 *
 * ВЫБОР: из ВЫБОРКИ случайных живых берём ту, что убивает меньше всего остальных
 * («беречь место для рождения»). Размер выборки задаётся: 1 = чистый случай.
 *
 * СБОРКА: cc -O3 -march=native -o witness3d witness3d.c
 * ЗАПУСК: ./witness3d <n> <цель> <перезапусков> [размер-выборки] [семя]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#define MAXW 32   /* было 16 = 1024 клетки. При n=11 клеток 1331, при n=12 — 1728:
                     * NW превышал MAXW и слово писалось за конец BS. Проверка NW>MAXW стояла
                     * и отсекала запуск, но буфер buf[1024] ниже НЕ проверялся ничем. */
typedef struct { unsigned long long w[MAXW]; } BS;

static int N, NN, NW;
static signed char *PX,*PY,*PZ;
static BS *killp;

static inline void bs_set(BS*a,int i){ a->w[i>>6]|=1ULL<<(i&63); }
static inline int  bs_get(const BS*a,int i){ return (a->w[i>>6]>>(i&63))&1ULL; }
static inline void bs_clr(BS*a,int i){ a->w[i>>6]&=~(1ULL<<(i&63)); }
static inline void bs_andnot(BS*a,const BS*b){ for(int t=0;t<NW;t++) a->w[t]&=~b->w[t]; }
static inline int  bs_pop(const BS*a){ int s=0;for(int t=0;t<NW;t++) s+=__builtin_popcountll(a->w[t]);return s; }

static unsigned long long rs;
static inline unsigned long long rnd(void){ rs^=rs<<13; rs^=rs>>7; rs^=rs<<17; return rs; }

/* список живых клеток */
static int alive_list(const BS*a,int*out){
    int c=0;
    for(int t=0;t<NW;t++){
        unsigned long long w=a->w[t];
        while(w){ int b=__builtin_ctzll(w); w&=w-1; out[c++]=(t<<6)+b; }
    }
    return c;
}

/* положить точку i при уже выбранных chosen[0..k-1]: вернуть новый живой набор.
 * Плоскости снимаются ТОЛЬКО с живых клеток — их к концу десятки. */
static void place(const BS*alive,int i,const int*chosen,int k,BS*out){
    *out=*alive;
    bs_clr(out,i);
    for(int t=0;t<k;t++) bs_andnot(out,&killp[(size_t)i*NN+chosen[t]]);
    if(k<2) return;
    static int buf[MAXW*64]; int m=alive_list(out,buf);  /* было buf[1024] — молчаливое переполнение при NN>1024 */
    for(int a=0;a<k;a++){
        long long ux=PX[chosen[a]]-PX[i], uy=PY[chosen[a]]-PY[i], uz=PZ[chosen[a]]-PZ[i];
        for(int b=a+1;b<k;b++){
            long long vx=PX[chosen[b]]-PX[i], vy=PY[chosen[b]]-PY[i], vz=PZ[chosen[b]]-PZ[i];
            long long nx=uy*vz-uz*vy, ny=uz*vx-ux*vz, nz=ux*vy-uy*vx;
            if(nx==0&&ny==0&&nz==0) continue;
            for(int q=0;q<m;q++){
                int t=buf[q];
                if(t<0) continue;
                long long wx=PX[t]-PX[i], wy=PY[t]-PY[i], wz=PZ[t]-PZ[i];
                if(nx*wx+ny*wy+nz*wz==0){ bs_clr(out,t); buf[q]=-1; }
            }
        }
    }
}

int main(int argc,char**argv){
    if(argc<4){ fprintf(stderr,"usage: witness3d <n> <цель> <перезапусков> [выборка] [семя]\n"); return 2; }
    int n=atoi(argv[1]), target=atoi(argv[2]);
    long long tries=atoll(argv[3]);
    int SAMP=(argc>4)?atoi(argv[4]):8;
    rs=(argc>5)?(unsigned long long)atoll(argv[5]):88172645463325252ULL;
    N=n; NN=n*n*n; NW=(NN+63)/64;
    if(NW>MAXW){ fprintf(stderr,"n слишком велико\n"); return 2; }

    PX=malloc(NN);PY=malloc(NN);PZ=malloc(NN);
    { int t=0; for(int x=0;x<n;x++)for(int y=0;y<n;y++)for(int z=0;z<n;z++){PX[t]=x;PY[t]=y;PZ[t]=z;t++;} }
    killp=malloc((size_t)NN*NN*sizeof(BS));
    if(!killp){ fprintf(stderr,"нет памяти\n"); return 2; }
    memset(killp,0,(size_t)NN*NN*sizeof(BS));
    for(int i=0;i<NN;i++)
        for(int j=i+1;j<NN;j++){
            BS m; memset(&m,0,sizeof m);
            long long ux=PX[j]-PX[i],uy=PY[j]-PY[i],uz=PZ[j]-PZ[i];
            for(int k=0;k<NN;k++){
                if(k==i||k==j) continue;
                long long vx=PX[k]-PX[i],vy=PY[k]-PY[i],vz=PZ[k]-PZ[i];
                if(uy*vz-uz*vy==0&&uz*vx-ux*vz==0&&ux*vy-uy*vx==0) bs_set(&m,k);
            }
            killp[(size_t)i*NN+j]=m; killp[(size_t)j*NN+i]=m;
        }

    int *chosen=malloc(sizeof(int)*NN), *best=malloc(sizeof(int)*NN), *lst=malloc(sizeof(int)*NN);
    int bestk=0;
    struct timespec t0,t1; clock_gettime(CLOCK_MONOTONIC,&t0);
    for(long long tr=0;tr<tries;tr++){
        BS alive; memset(&alive,0,sizeof alive);
        for(int i=0;i<NN;i++) bs_set(&alive,i);
        int k=0;
        while(1){
            int cnt=alive_list(&alive,lst);
            if(!cnt) break;
            int pick=-1; BS bestset; int bestleft=-1;
            int s=(SAMP<cnt)?SAMP:cnt;
            for(int q=0;q<s;q++){
                int cand=lst[rnd()%(unsigned long long)cnt];
                BS na; place(&alive,cand,chosen,k,&na);
                int left=bs_pop(&na);
                if(left>bestleft){ bestleft=left; pick=cand; bestset=na; }
            }
            if(pick<0) break;
            alive=bestset; chosen[k++]=pick;
        }
        if(k>bestk){
            bestk=k; memcpy(best,chosen,sizeof(int)*k);
            clock_gettime(CLOCK_MONOTONIC,&t1);
            double el=(t1.tv_sec-t0.tv_sec)+1e-9*(t1.tv_nsec-t0.tv_nsec);
            printf("НАЙДЕНО %d (перезапуск %lld, %.1fс):",k,tr,el);
            for(int t=0;t<k;t++) printf(" (%d,%d,%d)",PX[best[t]],PY[best[t]],PZ[best[t]]);
            printf("\n"); fflush(stdout);
            if(k>=target) break;
        }
    }
    clock_gettime(CLOCK_MONOTONIC,&t1);
    double el=(t1.tv_sec-t0.tv_sec)+1e-9*(t1.tv_nsec-t0.tv_nsec);
    printf("ИТОГ n=%d: лучшее %d (цель %d), %.1fс\n",n,bestk,target,el);
    return 0;
}
