...
#define MAXORB 9000
static P orb[MAXORB][3]; static int orbsz[MAXORB], orbsum[MAXORB], norb=0;
...
static void dfs(int *live,int nlive){
    ...
    for(int i=0;i<nlive;i++){
        ...
        if(ORD){                                   /* переупорядочить по близости суммы к середине */
            int mid=3*(n-1)/2;
            for(int a=1;a<nn;a++){ int v=nl[a], ka=orbsum[v]>mid?orbsum[v]-mid:mid-orbsum[v], b=a-1;
                while(b>=0){ int kb=orbsum[nl[b]]>mid?orbsum[nl[b]]-mid:mid-orbsum[nl[b]];
                    if((ORD==1)?(kb<=ka):(kb>=ka)) break; nl[b+1]=nl[b]; b--; }
                nl[b+1]=v; } }
        dfs(nl,nn); free(nl);
        depth0--;
        npts=save;
...
int main(int argc,char**argv){
    ...
    for(int x=0;x<n;x++)for(int y=0;y<n;y++)for(int z=0;z<n;z++){
        ...
        orbsz[norb]=k; orbsum[norb]=x+y+z; norb++; }
