...
int main(int argc,char**argv){
    ...
    /* ВЫРОЖДЕННЫЙ СЛУЧАЙ НАЗЫВАЕТСЯ СЛОВАМИ. Когда best остался равен BEST0, строка
     * «МАКСИМУМ 35» есть ЭХО вложенного порога, а не измерение: о достижимости самого
     * порога она не говорит ничего. Данные для этого вывода в строке были и раньше
     * («нач.лучшее=35» стояло рядом), и я всё равно прочёл эхо как результат. Значит
     * добавлять данные бесполезно — надо, чтобы вырожденный случай СЕБЯ НАЗЫВАЛ. */
    if(BEST0>0 && best==BEST0){
        printf("n=%d орбит=%d доля=%d/%d.%d/%d нач.лучшее=%d: %s НЕ ПРЕВЗОШЁЛ НАЧАЛЬНОГО "
               "(о достижимости самого %d НИЧЕГО НЕ СКАЗАНО — нужна отдельная калибровка "
               "с порогом на единицу ниже), узлов %lld, %.1fс\n",
               n,NORB,SHARD,NSHARD,SHARD2,NSHARD2,BEST0, stopped?"ОБОРВАНО —":"ИСЧЕРПАНО —",
               BEST0, nodes, elapsed());
        return 0;
    }
