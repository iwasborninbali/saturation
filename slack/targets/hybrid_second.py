...
def solve(name, col):
    """Возвращает True, если поддерево закрыто целиком."""
    if col >= MAXC:
        stats["предел"] += 1
        print(f"ПРЕДЕЛ {name} (столбцы исчерпаны на {col})", flush=True)
        return False
    u = units(name)
    f = f"{OUT}/{shard}.cnf"
    head = b"p cnf %s %d\n" % (NV, NCL + len(u))
    ...
    ok = True
    for j in range(64):
        if not solve(f"{name}_c{col}s{j:03d}", col + 1): ok = False
    return ok
