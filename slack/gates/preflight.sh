...
probe () {  # печатает: cpus busy freeGB busy_kissat
  if [ "$T" = local ]; then
    ...
    # Разбор через NF-1 давал слово «idle», а не число: строка top оканчивается пробелом,
    # поэтому последнее поле пустое. В итоге ворота печатали «загрузка 8.0» ВСЕГДА и отказывали
    # на пустой машине весь день. Ворота, отказывающие всегда, сломаны так же, как пропускающие
    # всегда, — просто их поломка безопасна и потому незаметна.
    # Берём ВТОРУЮ выборку: первая на macOS считается от загрузки системы и недостоверна.
    idle=$(top -l 2 -n 0 2>/dev/null | grep '^CPU usage' | tail -1 | grep -oE '[0-9.]+% idle' | grep -oE '^[0-9.]+')
    case "$idle" in ''|*[!0-9.]*) idle= ;; esac
