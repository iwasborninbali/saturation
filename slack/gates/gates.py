...
def gate_fact_sources(paths, problem="a280537", n=7, M=19):
    """ИСТОЧНИКИ ФАКТОВ: ЯВНЫЙ СПИСОК + ОБЪЯВЛЕННАЯ ЗАДАЧА.

    21 августа самопополняющийся цикл первого решателя объявил решающий вопрос ЗАКРЫТЫМ —
    ложно. Он набирал источники по образцу logs/*.txt, и туда попали 257 закрытий ЗАДАЧИ n=5
    о трёх точках на прямой. Её куски называются так же: case_00000, case_00001... Ничто
    в имени case_00007 не говорит, к какой задаче оно относится. Вердикт перевернулся
    с «63 из 64» на «закрыто целиком», мера показала ровно 1.000000000000.

    У меня в тот же час нашлось то же самое, мягче: образец facts_*.txt втягивал
    facts_ОТОЗВАНЫ_столкновение_имён.txt, и 280 сознательно отозванных имён засчитывались
    как закрытые.

    НАПРАВЛЕНИЕ ЭТОЙ ОШИБКИ ОПАСНОЕ. Все прежние дефекты имён ЗАНИЖАЛИ покрытие: узел под
    двумя написаниями не набирал детей, и мы теряли работу, но не лгали. Здесь чужие закрытия
    ДОБАВЛЯЮТСЯ, то есть покрытие ЗАВЫШАЕТСЯ, и инструмент говорит «доказано» там, где нет.

    Что спасло первого решателя: противоречие между мерой и памятью о вчерашнем числе. То есть
    спас не сторож, а воспоминание — самая ненадёжная защита из возможных. Отсюда ворота.

    Требуют от КАЖДОГО источника заголовочную строку с объявлением задачи:
        # ЗАДАЧА: a280537 n=7 M=19
    Файл без объявления отвергается: молчание источника о своей задаче — не согласие.
    """
    import os, re
    if isinstance(paths, str): paths = [paths]
    need = f"{problem} n={n} M={M}"
    отчёт = {"итог": None, "источники": [], "отвергнуты": []}
    for p in paths:
        base = os.path.basename(p)
        if not os.path.exists(p):
            отчёт["отвергнуты"].append((base, "файла нет — это НЕ пустой источник")); continue
        if re.search(r"ОТОЗВ|отозв|withdraw|карантин|quarant", base, re.I):
            отчёт["отвергнуты"].append((base, "имя файла объявляет карантин — источником быть не может")); continue
        # МАРКЕРЫ СЛИЯНИЯ. Файл фактов ПОРОЖДЁННЫЙ, он конфликтует при каждом одновременном
        # обновлении, и разрешать такой конфликт нельзя в принципе: смешав два порождённых
        # содержимого, получишь то, что не является верной выгрузкой НИ ОДНОЙ стороны — при
        # том что выглядит правдоподобно и проходит любую проверку формата. 21 августа
        # маркеры полчаса лежали на origin в файле фактов первого решателя, и строка
        # "<<<<<<< Updated upstream" читалась бы как имя узла тем, кто о ней не спотыкается.
        # Порождённый файл чинится ПЕРЕГЕНЕРАЦИЕЙ, а не правкой.
        with open(p, encoding="utf-8", errors="replace") as fh:
            марк = sum(1 for l in fh if l.startswith(("<<<<<<<", ">>>>>>>")) or l.rstrip("\n") == "=======")
        if марк:
            отчёт["отвергнуты"].append((base, f"МАРКЕРЫ СЛИЯНИЯ ({марк}) — файл не является выгрузкой ни одной стороны; перегенерировать, не править"))
            continue
        декл = None
        with open(p, encoding="utf-8", errors="replace") as fh:
            for i, l in enumerate(fh):
                if i > 40: break
                m = re.search(r"ЗАДАЧА:\s*(.+)", l)
                if m: декл = m.group(1).strip(); break
        if декл is None:
            отчёт["отвергнуты"].append((base, "нет строки «# ЗАДАЧА:» — молчание не есть согласие")); continue
        if need not in декл:
            отчёт["отвергнуты"].append((base, f"объявлена ДРУГАЯ задача: {декл!r}, нужна {need!r}")); continue
        cnt = sum(1 for l in open(p, encoding="utf-8", errors="replace")
                  if l.startswith("case_"))
        отчёт["источники"].append({"файл": base, "задача": декл, "имён": cnt})
    отчёт["итог"] = "ЧИСТО" if not отчёт["отвергнуты"] else "ОТВЕРГНУТ ИСТОЧНИК"
    if not отчёт["источники"]:
        отчёт["итог"] = "НЕТ ГОДНЫХ ИСТОЧНИКОВ"
    return отчёт
