...
echo "== 4. ИНСТРУМЕНТЫ, которые называет документация"
# Битая ссылка на файл и отсутствующий инструмент — одна ошибка: документ велит сделать то,
# чего сделать нельзя. drat-trim у нас назван восемь раз и подкрепляет ЦЕНТРАЛЬНУЮ проверку,
# а 2026-08-20 обнаружилось, что его нет ни на одной машине.
# ТОЛЬКО документы-инструкции. Первая версия читала docs/*.md подряд и потребовала cadical,
# который упомянут в ЦИТАТЕ чужой статьи и в исторической переписке по другой задаче.
# Проверять надо то, что велено сделать ПРОВЕРЯЮЩЕМУ, а не всё, что где-либо названо.
for t in $(grep -ohE "\b(drat-trim|kissat|cadical|glucose|minisat)\b" docs/VERIFICATION.md docs/HANDOVER.md docs/research/ADVERSARIAL_REVIEW_BRIEF.md 2>/dev/null | sort -u); do
  if command -v "$t" >/dev/null 2>&1; then echo "  ок: $t"
  else echo "  НЕТ ИНСТРУМЕНТА: $t — документация на него ссылается, воспроизвести нельзя"; fail=1; fi
done
