# Чеклист внешней/человеческой проверки заметки hjsw_window v1.13 (что проверять и чем)
...
## Устойчивость одной гиперболы (v1.12; второй солвер) — `paper/lemma_stability.tex`
- Lemma orbdec (орбитное разложение): ДОКАЗАТЕЛЬСТВО в две строки из cor:slopes + lem:lagrange — проверить руками; численный контроль
  `slack/t221/orbit_decomp_one.py` (0 прямых ЛЮБОГО наклона пересекают орбиту при p ≤ 19).
- Lemma orbopt (одна орбита): ключевое — «типов орбит ровно два». Проверить рассуждение: генерическая орбита = {A,A,D,D} или {B,B,C,C} (из lem:partners),
  изоморфны при (x,y) ↦ (y,x); профиль богатых прямых (4,4,3,3,3,3). Численно: `slack/t221/orbit_types.py` (ровно 2 типа, p ≤ 19),
  `slack/t221/orbit_stability.py` (оптимум/число максимумов/c(d) для всех орбит всех p ≤ 31).
- Prop stability: |S \ M| ≤ t — арифметика суммирования по орбитам (три строки).

## Блочное разложение (v1.11–v1.13) — `paper/section_blocks.tex` + `paper/lemma_run.tex` (первый солвер)
- Lemma edges: точка ↔ ребро {t, t+1} пары КВ; 8 классов = 32 точки (вырожденные рёбра при t = 0, −1 — 16 точек). Проверить вывод
  a = u_i + u_{i+1}, 1/a = u_{i+1} − u_i и подсчёт классов.
- Theorem blocks: разбиение + локальность СТРОК, СТОЛБЦОВ и ±1‑ПРЯМЫХ (не всех прямых! смешанные 3‑точечные прямые общих наклонов могут пересекать
  блок — это явно оговорено; оценка от этого лишь ослабевает). Численно: `slack/t221/decomposition_check.py` (partition=True, 0 пересечений, Σ LP = LP(1)).
- Lemma run + Prop descent (первый солвер): кривая C_k, род 1 + 2^{k−2}(k−3), закон спусков D_k(S)/k!; точное тождество через sin·sin.
- Corollary blockconst: таблица s(ε) ТРЕМЯ независимыми способами (`slack/t221/fast_blocks.py`, `slack/sig_savings.py`, `slack/t221/standard_model.py`),
  500 сравнений при k ≤ 9, 0 расхождений; свёртка `slack/t221/constant.py` / `constant2.py` / `slack/constant_km1.py`.
  ВАЖНО при проверке: стандартная модель берёт МИНИМУМ по генерическим позициям (вырожденные позиции дают БОЛЬШУЮ экономию — это безопасно).
- Remark (c): метод работает только при k = ±1 (`slack/blocks_general_k.py`); Remark (d): в O ∪ R(O) максимум ровно тривиальный (`slack/orbit_union.py`)
  ⇒ локальных сертификатов для 3(p−1) не существует.

## Что НЕ доказано (и так и написано)
- Пара гипербол: истинное значение ≈ 3(p−1)+O(1) (данные); доказано 427101431/123863040 = 3.44817; модель вертикальных пар — условная теорема (Wigner‑край, CH_δ).
