...
(uniform on {0,1,2}, independent): z = (u* + r p)/3 with r ≡ ρ (mod 3) [p ≡ 2 mod 3 ⇒ 3 | u* + rp iff r ≡ u* mod 3], i.e. z/p = (s + r)/3 as a real number,
   ...
   thresholds: θ₊(x_m) = 1 − frac(F_m − pos(x_m) − y₀/p),   θ₋(x_m) = frac(F_m + pos(x_m) − y₀/p)   [x = x₀ + p·pos(x), so no extra x₀ term;
   corrected 19.08 after agent B's check against `cube_matching_fast.py`],
...
lift of the shared point, its own (u′*, v′*) with the box constraints, and w″ = (a z′³ mod p)/p.  Digits (exact rules): (i) z′ = z/4 (mod p) means
z′ = (z + jp)/4 with j ∈ {0,1,2,3} determined by z mod 4, i.e. by u* mod 12 (z = (u*+rp)/3, r ≡ u* mod 3), so u* mod 12 (uniform on 12 classes,
independent of s) replaces ρ; then z′/p = ((s+r)/3 + j)/4 exactly; likewise /5 uses u* mod 15, and −4z/5, −5z/4 use both.  (ii) w″: a z′³ ≡ 64⁻¹·(a z³) for
z′ = z/4, and W := a z³ mod p has position w′ = W/p; 64⁻¹W ≡ (W + ip)/64 with i ∈ {0,…,63} determined by W mod 64 — a fine digit, uniform and independent
of w′ asymptotically — so w″ = (w′ + i)/64; for /5: w″ = (w′ + i)/125, i uniform on {0..124}; for the integer ratios w″ = frac(64 w′) resp.\ frac(−125 w′)
(no digit); for −4z/5: w″ = frac(−64·(w′+i)/125).  Averaging over the digits gives γ₂ as a finite sum of two-dimensional integrals in (s, w′).  The five residues (shared one and two
