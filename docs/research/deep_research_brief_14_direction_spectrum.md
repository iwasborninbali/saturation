...
## Дополнение 3.09.2026 (после публикации заметки, DOI 10.5281/zenodo.22275038) — что искать ПРЕЖДЕ ВСЕГО
1. **Модель прямых.** Известна ли модель: точки no-three-in-line конфигурации, разложенные по прямым одного направления с ограничением
   «≤ 2 на прямой» и весами C(L, x) по клеткам (канонический ансамбль жёстких частиц в ящиках ёмкости 2), с перенормировкой на число пар?
   Любая работа, где спектр направлений (число пар точек по примитивным направлениям) решений измерен или выведен — Flammenkamp
   (JCTA 1992, 1998, его страницы), Guy–Kelly 1968 (они суммируют по направлениям ожидания коллинеарных троек в равномерной модели —
   считали ли пары при «≤ 2»?), Hall–Jackson–Sudbery–Wild 1975, Erdős, Roth; статьи о «direction distribution» точечных множеств
   (Ungar, Scott, Jamison — число направлений, определяемых n точками — другой вопрос, но соседний).
2. **Измерения ансамбля решений.** Кто-нибудь публиковал статистику решений no-three-in-line (распределение по направлениям, число
   дважды занятых диагоналей, «κ»)? Flammenkamp'овская страница симметрий; Pegg; Cooper–Pikhurko–Schmitt–Warrington (Gardner's minimum).
3. **Решёточный газ ёмкости 2.** Стандартные названия/источники для ансамбля «независимые ящики ёмкости k с вырождениями C(L, x)»
   (hard-core lattice gas, rook polynomial of disjoint blocks) — чтобы опознать тождество Теоремы 1 заметки по имени.
4. **Рост длинных направлений с n** (c(3,7): 0.179 при n≈20 → 0.221 при n≥32) — есть ли известное объяснение или измерение?
Ответ нужен с источниками и датами; «не найдено, искали там-то» — законный ответ.
