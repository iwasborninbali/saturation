...
## 30. LP(1) РАСКЛАДЫВАЕТСЯ (второй солвер, 19.08 11:40 WITA): компоненты графа соседей — точная декомпозиция, и IP по компонентам БЬЁТ LP(1)
Тулы: `slack/t221/decomposition_check.py`, `comp_zoo.py`, `comp_table.py`, `type_determinism.py`, `union_lp.py`, `window_rule.py`, `indep_set.py`.
Уточняет B.19 («LP(1) — глобальный объект»): он не глобальный, он локален на уровне КОМПОНЕНТ.

**Определения.** Для d ∈ F_p (включая d = 0) орбита Ω_d := {q ∈ P₋₁ : ±d ∈ {r₊(q), r₋(q)}}, r₊ = x−y, r₋ = x+y (mod p); Ω_d = Ω_{−d}, индексируем
x = d². Граф соседей: x ~ x+4 (Lemma neighbours раздела 5: Ω_d ∩ Ω_e ≠ ∅ ⇔ e² ≡ d² ± 4). Компоненты — пути (каждая вершина имеет ≤ 2 соседей).
Вершина существует ⇔ группа непуста ⇔ d²+4 или d²−4 — квадрат (плотность 3/4), поэтому средняя длина компоненты ≈ 4, хвост геометрический.

**ФАКТ 1 (разбиение).** Точечные множества компонент РАЗБИВАЮТ P₋₁ (каждая точка q лежит ровно в двух орбитах, при x = r₊(q)² и x = r₋(q)²,
и они соседи, т.к. r₋² − r₊² = ±4 ⇒ одна компонента). Проверено при p = 113, 137, 199, 401: partition = True.
**ФАКТ 2 (замкнутость прямых).** КАЖДАЯ прямая с ≥ 3 точками (строка, столбец, ±1‑прямая) лежит внутри одной компоненты — 0 исключений при
p = 113, 137, 199, 401 (при исключении d = 0 появлялись ровно 4 «пересекающие» прямые вычета 0 — это и был недостающий узел цепи 4 — 0 — (−4)).
**ФАКТ 3 (точная декомпозиция LP(1)).** Σ_C LP(U_C) = LP(1) с точностью до машинного нуля (p = 137, 199, 401). Т.е. рангово‑1 сертификат
со строками/столбцами/±1‑прямыми — прямая сумма по компонентам.
**ФАКТ 4 (целочисленность внутри компонент бьёт LP(1)).** α(P₋₁) ≤ Σ_C IP(U_C) (компоненты разбивают P, прямые не пересекают границ):
   p=137: 460 против LP(1) = 460.67 (3.3824 против 3.3873 (p−1));  p=199: 680 против 684 (3.4343 против 3.4545);  p=401: 1364 против 1372 (3.4100 против 3.4300).
**ФАКТ 5 (экономия зависит только от типа компоненты).** Сигнатура = последовательность профилей размеров ±1‑прямых вдоль цепи;
на 139 компонентах при p = 199, 401, 499 — 23 различных сигнатуры, НИ ОДНОЙ неоднозначной (одна и та же сигнатура ⇒ одна и та же экономия).
Зоопарк (экономия = |U|/2 − IP(U)): «8» = 8; «7» = 4; «77» = 7 (LP 20/3, целочисленность даёт 7); «78» = 8 (семь‑орбита рядом с хорошей НЕ добавляет
ничего); «777» = 10; «778» = 12; «787» = 8; «6» между двумя 2‑классовыми = 6; «686» = 16; «6о6» и т.д. — таблица в `slack/t221/comp_savings.json`.
**ФАКТ 6 (константа).** Суммарно по простым 401…1103: экономия/p = 0.5587 ⇒ **α ≤ 3.4413(p−1)** эмпирически — это и есть «потолок 3.45» из B.7,
достигнутый РАЗЛОЖИМЫМ сертификатом (и чуть ниже LP(1) за счёт целочисленности внутри компонент).
Усечение по длине цепи: L ≤ 3: 3.72; L ≤ 5: 3.595; L ≤ 7: 3.480; L ≤ 8: 3.470; L ≤ 10: 3.452; L ≤ 11: 3.4419 (хвост дальше ≈ 0.0006).

**Что нужно для ТЕОРЕМЫ (α ≤ (c + o(1))(p−1), c ≈ 3.44).**
(а) Леммы 1–2 доказать (обе — двухстрочные следствия r₋² − r₊² = ±4 и леммы соседей; замкнутость строк/столбцов уже есть в lem:klein).
(б) Плотности сигнатур: цепь x, x+4, …, x+4(L−1) с предписанными типами групп — события «d²±4 квадрат» и знаковые условия положений классов;
    это ровно машина prop:m8asym/prop:m7asym на кривой с L квадратичными расширениями (род растёт линейно по L) ⇒ плотность каждой сигнатуры
    длины L вычислима с ошибкой O_L(√p log^{c}p). Конечное усечение (L ≤ L₀) даёт честную теорему с константой 3.72 / 3.595 / 3.48 / 3.47 / 3.452…
(в) Лучше: ЛОКАЛЬНОЕ ПРАВИЛО. Аддитивная модель по окнам радиуса 1 НЕ точна (max |err| = 1.66 на 38 типах, `window_rule.py`) — нужен трансфер‑матричный
    (DP) вид: IP(U_C) как динамика вдоль цепи с конечным числом состояний. Если DP найден и доказан, константа = среднее по цепи Маркова
    с переходами из локального закона — теорема без усечения. ЭТО ГЛАВНАЯ ОТКРЫТАЯ ТЕХНИЧЕСКАЯ ЗАДАЧА этого вектора.
**Смысл.** Ответ на вопрос хозяина «как достичь 3.45»: не полировкой блоков, а декомпозицией. Потолок локальных сертификатов достигается
разложимым по компонентам аргументом; ниже 3.44 этим способом не спуститься (это ровно IP(1)), и остаток до 3(p−1) несут 3‑точечные прямые
общих наклонов (явление из `docs/research/integrality/phenomenon.py`).
