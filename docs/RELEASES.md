...
## hjsw-note-v1.13.1 — 2026-08-19 12:58 WITA
- Lemma orbdec/orbopt: доказательство переписано через уже имеющееся Prop lines (полный список богатых прямых) — универсальность структуры орбиты теперь ВЫВОДИТСЯ, а не постулируется; аннотация: та же поправка, что в теореме о блоках (строки/столбцы/±1‑прямые, а не «любая богатая прямая»).
- Remark (a) леммы устойчивости: оценка резка при каждом t — max |S \\ M| = min(t, p − 1 − 2s) (CP‑SAT, p = 11…31, t ≤ 4, каждое значение доказано оптимальным; независимая третья реализация). Лемма проверена тремя независимыми реализациями.
- slack/t221/standard_model.json (510 паттернов, k ≤ 9) закоммичена: свёртка из репозитория воспроизводит 68350729/123863040 точно.
- SHA-256(paper/hjsw_window.pdf) = abed25fc55db3d4dc96238ed7f742160c1cf6af3d58cece37aaad12e39a58db3

## hjsw-note-v1.14 — 2026-08-19 23:28 WITA
- По итогам дип‑ресёрча 8 (23 агента; ответ в `docs/research/integrality/deep_research_8_answer.md`, 2148 строк, плюс исполняемый сьют из 198 тестов):
  • раздел «Computational observations» — количественная калибровка чтения «выигрыш = O(1)»: подгонка gain = a + c(p−1) даёт c ≈ 0.06 (пара) и
    c ≈ 0.05 (четвёрка) против c ≈ 0.44, нужных для превосходства над HJSW ⇒ эффект исключён почти на порядок, НО данные не различают
    ограниченный выигрыш и линейный с наклоном ≤ 0.1; рядом — предостерегающий пример из наших же данных (межблочный дефицит 0, 8, 5, 5 при
    p = 11…19 выглядит константой, хотя доказуемо растёт как 0.44(p−1));
  • иерархия сертификатов уточнена: 3.45 — это LP(1), а не LP(all); измеренный LP(all) в среднем 3.396(p−1), его разброс объясняется
    флуктуацией m₈ (corr −0.79), предел ≈ 3.37(p−1);
  • зафиксировано отрицательно: четвёрка гипербол НЕ бьёт 3/2·N; переносов из других систем ресёрч не нашёл; H4(ii) невозможна алгебраически;
    H3 (устойчивость) LP‑невидима; разрыв целочисленности рождается ровно при склейке пары в коробке, единственный носитель — смешанные 2+1 прямые.
- SHA-256(paper/hjsw_window.pdf) = 7c86f05afd0e61e236cb077dc6b8b3435831480100851d056ef6fbf83a299f8e

## defects-v1.0 — 2026-08-20 01:28 WITA
- n = 41 ЗАКРЫТ ПОЛНОСТЬЮ: все 4750 подклассов исчерпаны в каждой из двух баз, ноль реальных таймаутов, конфигураций нет (журналы logs/sweeps/f41A_sym{9,10}_n41.txt).
