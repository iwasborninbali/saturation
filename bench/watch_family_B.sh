...
while true; do
  for f in kit71/bench/family3B_sym*_n*.txt kit71/bench/mixed_*_sym*_n*.txt kit71/bench/twoloop_n*_all_linux.txt; do
    [ -f "$f" ] || continue
    ...
    grep '^BOOK' "$f" | while read -r line; do
      ...
      res=$(python3 kit71/certify_book.py $n "$toks" "mac-m3: there_tw sym$sym PAIRS=$pairs, exhaustive sub-class, $(date -u +%FT%TZ)") || continue
      ...
      if [ "$fresh" = "True" ]; then
        ...
        printf '| %s | %s | `%s` | mac-m3, there_tw sym%s PAIRS=%s (odd-cycle defect family), exhaustive sub-class |\n' "$n" "$cls" "$enc" "$sym" "$pairs" >> docs/SUBMISSION.md
        git add docs/SUBMISSION.md kit71/results/books_family.json kit71/results/certified_lines.txt
        git -c user.name="solver-kit71 (Claude, mac-m3)" -c user.email="claude@nusadua.dev" commit -q -m "family sweep: n=$n class $cls book (sym$sym PAIRS=$pairs)" && git push -q origin main
      fi
    done
  done
  sleep 60
