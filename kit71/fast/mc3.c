...
/* buckets of cells by F value (capped at NB-1) */
#define NB 12
static int bucket[NB][MAXC], bsize[NB], bpos[MAXC], bval[MAXC];
static inline void bucket_move(int c, int nv) {
    int ov = bval[c];
    if (ov == nv) return;
    int i = bpos[c], last = bucket[ov][--bsize[ov]];
    bucket[ov][i] = last; bpos[last] = i;
    bval[c] = nv; bpos[c] = bsize[nv]; bucket[nv][bsize[nv]++] = c;
}
static inline void bucket_init(void) {
    for (int v = 0; v < NB; v++) bsize[v] = 0;
    for (int c = 0; c < n * n; c++) { bval[c] = 0; bpos[c] = bsize[0]; bucket[0][bsize[0]++] = c; }
}
static int scanned_mark[MAXC]; static int scan_stamp = 0;
...
static inline void walk(int a, int b, int d) {
    ...
    while (u >= 0 && u <= m && v >= 0 && v <= m) {
        int c = u * n + v; F[c] += d;
        int nv = F[c] < NB - 1 ? F[c] : NB - 1;
        if (nv != bval[c]) bucket_move(c, nv);
        u += du; v += dv;
    }
}
...
int main(int argc, char **argv) {
    ...
    double p3 = getenv("P3") ? atof(getenv("P3")) : 0.0;
    ...
    long STUCK = getenv("STUCK") ? atol(getenv("STUCK")) : 0;       /* iterations without a new lawful best before reload; 0 = off */
    long last_lawful_it = 0;
    ...
    for (;;) {
        ...
        if (conf == 0) {
            if (np > best_lawful) {
                best_lawful = np; last_lawful_it = it;
        ...
        if (RELOAD && nbest_tokens > 0 && (it % RELOAD) == 0 && (conf > DRIFT || (STUCK && it - last_lawful_it > STUCK))) {
            if (STUCK && it - last_lawful_it > STUCK) last_lawful_it = it;
        ...
        int kk = 1;
        { double x = runif(); if (x < p3) kk = 3; else if (x < p3 + p2) kk = 2; }
        if (np >= 2 * n && kk == 1) kk = 2;
        int rr_[3]; rr_[0] = r1; int nrr = 1;
        while (nrr < kk && nrr < nreps) { int c = reps[rint_(nreps)]; int dup = 0; for (int i = 0; i < nrr; i++) if (rr_[i] == c) dup = 1; if (!dup) rr_[nrr++] = c; }
        kk = nrr;

        long before = conf;
        for (int i = 0; i < kk; i++) remove_orbit(rr_[i]);
        ...
        for (int step = 0; step < kk; step++) {
            ...
            if (runif() < noise) {
                ...
                scan_stamp++;
                for (int v = 0; v < NB; v++) {
                    if (v > bestp) break;                     /* proxy >= F of any of its cells */
                    for (int bi = 0; bi < bsize[v]; bi++) {
                        int cell = bucket[v][bi];
                        int c = orbrep[cell];
                        if (scanned_mark[c] == scan_stamp) continue;
                        scanned_mark[c] = scan_stamp;
                        if (c == r1 || repidx[c] >= 0) continue;
        ...
        int complete = (nins == kk);
        if (complete && (delta <= 0 || runif() < exp(-delta / T))) {
            acc++;
            if (TABU) { for (int i = 0; i < kk; i++) tabu_until[rr_[i]] = it + TABU + rint_((int)TABU + 1); }
        }
        else {
            for (int i = nins - 1; i >= 0; i--) remove_orbit(ins[i]);
            for (int i = kk - 1; i >= 0; i--) insert_orbit(rr_[i]);
        }
    }
    if (full_conf() != conf) { fprintf(stderr, "conf mismatch at end\n"); return 3; }
    for (int c = 0; c < n * n; c++) { int nv = F[c] < NB - 1 ? F[c] : NB - 1; if (bval[c] != nv || bucket[bval[c]][bpos[c]] != c) { fprintf(stderr, "bucket mismatch at %d\n", c); return 3; } }
