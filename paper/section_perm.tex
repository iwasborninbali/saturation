...
Lemma~\ref{lem:positions}, by linear disjointness); the tie \eqref{eq:tie} is an identity, not a hypothesis.  Finally $\mathrm{Res}(D_+,D_-)\ne0$ for \emph{every} odd $k$: a common root $x$ of $D_+$ and $D_-$ (over $\overline{\mathbb Q}$; the
resultant is an integer) would satisfy $(x^k+x)^{k-1}=-(x^k-x)^{k-1}$, so $x^k+x=\zeta(x^k-x)$ with $\zeta^{k-1}=-1$, whence $x\ne0$ and
$x^{k-1}=-(1+\zeta)/(1-\zeta)$; then $(x^k-x)^{k-1}=x^{k-1}(x^{k-1}-1)^{k-1}=\pm2^{k-1}(1+\zeta)/(1-\zeta)^{k}$ is a unit at every prime $\ell\mid k$
(the roots of unity $\zeta,-\zeta\ne1$ have order dividing $2(k-1)$, coprime to $\ell$, so $1\mp\zeta$ are $\ell$-adic units), whereas $D_+(x)=0$
demands $(x^k-x)^{k-1}=(k-1)^{k-1}/k^k$, of negative $\ell$-adic valuation — a contradiction.  (Symbolically, for $k=5$ the prime factors of the
resultant below $5000$ are $2,5,7,97,127$; for $k=7$: $2,3,5,7,193,1327$.)
\end{proof}
...
\]
\[
 C_9=\tfrac{94048104091}{33530112000}=2.80488\ldots,\qquad C_{11}=2.80489\ldots
\]
(\texttt{slack/pp\_constant\_exact.py}; for $k=3$ the two summands are $\tfrac12\cdot2$ and $\tfrac74$, as in Section~\ref{sec:cubic}).  As
$k\to\infty$ the rencontres numbers converge to the Poisson law $e^{-1}/j!$ ($|P_j-e^{-1}/j!|\le1/(j!\,(k-j+1)!)$), and $C_k\to C_\infty=2.80488\ldots$
(the value of \eqref{eq:Ck} for the Poisson law); since the first summand of \eqref{eq:Ck} is $16$-Lipschitz in $\ell^1(P)$ and the second is
$8$-Lipschitz in $\ell^1$ of the size-biased law $(jP_j)_j$, one gets $|C_k-C_\infty|\le2^{k+3}(k+5)/(k+1)!$, which is $<0.001$ for $k\ge11$; together
with the exact values for $k=7,9$ this gives $C_k<2.81$ for every odd $k\ge7$.

\begin{theorem}[permutation monomials]\label{thm:perm}
Let $k\ge3$ be odd, and let $p$ be a prime with $\gcd(k,p-1)=1$ and $p\nmid k(k-1)\mathrm{Res}(D_+,D_-)$ (a non-zero integer, Lemma~\ref{lem:twoslopes}).
Then for every $2p\times2p$ box $B$,
\[
 \alpha(x^k,B)\ \le\ \bigl(C_k+\varepsilon_k+o(1)\bigr)\,p,\qquad 0\le\varepsilon_k\le\frac2{(k-1)!}+\frac8{k!},\quad \varepsilon_3=0,
\]
as $p\to\infty$ ($k$ fixed); numerically $C_5+\varepsilon_5\le2.946$, $C_7+\varepsilon_7\le2.809$ and $C_k+\varepsilon_k<2.81$ for all odd $k\ge9$.  In particular
$\alpha(x^k,B)\le(1.474+o(1))\cdot2p<\tfrac32\cdot2p$ for every odd $k$: no permutation monomial graph reaches the HJSW value.
\end{theorem}
\begin{proof}
...
\begin{corollary}[generic permutation polynomials]\label{cor:permgen}
Let $k\ge3$ and let $f\in\Fp[x]$ be a permutation polynomial of degree $k$ such that for both $g_\pm(t)=f(t)\mp t$: (i) the map $t\mapsto g_\pm(t)$ has
monodromy group $S_k$ (for instance, $g_\pm'$ has $k-1$ distinct simple zeros with pairwise distinct critical values, and $p\nmid k$), and (ii) the
discriminants $\mathrm{disc}_t\bigl(g_+(t)-g_+(x)\bigr)$ and $\mathrm{disc}_t\bigl(g_-(t)-g_-(x)\bigr)$ are coprime polynomials in $x$.  Then for every
$2p\times2p$ box, $\alpha(f,B)\le(C_k+\varepsilon_k+o(1))\,p$ with the same $C_k$ and $\varepsilon_k$ as in Theorem~\ref{thm:perm} (the implied constants
depending on $k$ and on the family, uniformly in the box).
\end{corollary}
\begin{proof}
Nothing in Lemmas~\ref{lem:lifts}--\ref{lem:twoslopes} or in the proof of Theorem~\ref{thm:perm} used the shape of $f$ beyond (i) and (ii): the classes,
thresholds and the tie \eqref{eq:tie} are defined for any $f$ (as $g_-(x)-g_+(x)=2x$), the root-count laws are the rencontres numbers as soon as the
monodromy is $S_k$, the $j$-tuple curves are irreducible by $j$-transitivity, the only linear relation is $\sum_it_i=$ const for $j=k$ (covered by
$\varepsilon_k$), and (ii) gives the linear disjointness of the two slopes.  Hence the constant depends on $k$ only.
\end{proof}
For an arbitrary permutation polynomial (no assumption on the monodromy) the same proof gives $\alpha(f,B)\le(C(P^+,P^-)+\varepsilon+o(1))p$ with the
root-count laws $P^\pm$ of $f\mp x$ in place of the rencontres numbers, provided the corresponding root varieties are irreducible and the two slopes are
linearly disjoint; whether every permutation polynomial of degree $\ge3$ satisfies $\alpha\le(\tfrac32-c_k)N$ is open (see (c) below).
...
(b) Why the hyperbola $x\mapsto1/x$ sits exactly at $3/2$ in this framework: every residue has exactly two roots on each slope (no lines with $5$ or $6$
points), and since $c=t_1+t_2$ is a \emph{linear} function of the root pair, the same-class probability of a pair is $1/2$ rather than the $2/3$ of
Lemma~\ref{lem:positions}; the cover $W_4$ then costs $2\cdot\frac p2+2p=3p$ whatever the correlation between the two slopes (a residue contributes two
covered lifts for each slope on which its pair is same), in accordance with Theorem~\ref{thm:main}.  (Its two slopes are in fact maximally dependent —
the partners of $x$ are $\mp1/x$ and the pair of slope $+1$ shares its centre iff that of slope $-1$ does not — so Lemma~\ref{lem:twoslopes} would fail
for it; this affects only the double-coverage bookkeeping, not the value $3p$.)  For $x^k$ the residues with three or more roots and the same-probability
$2/3$ of the pairs are what push the constant below $3/2$.
