...
P\'or and Wood proved $a(n)=\Theta(n^2)$; no exact values appear to have been recorded, and the sequence does
not appear to be in the OEIS.  We determine
\[a(1),\dots,a(6)=1,\,8,\,16,\,28,\,40,\,64,\]
give certified lower bounds $a(7)\ge71$ and $a(8)\ge89$, and prove $a(p)\ge p^2$ for every prime $p$ by an
...
\section{Certified lower bounds for $n=7,8$}
\begin{center}\begin{tabular}{rrrl}\toprule
$n$ & $a(n)\ge$ & $/n^2$ & source of the witness\\\midrule
7 & 71 & 1.45 & randomised destroy-and-repair\\
8 & 89 & 1.39 & randomised destroy-and-repair\\\bottomrule
\end{tabular}\end{center}
Each configuration is re-verified by testing all $\binom{|S|}{3}$ triples with integer cross products.  These
are lower bounds only; we make no claim about how far they are from the truth, and we expect them to be weak,
since a randomised search is a poor instrument at this size.  (An earlier draft of this note quoted
...
For the record we also measured what the abandoned route would have cost.  A calibrated tree-size estimator
--- Knuth's random-probe method, stratified after the plain version proved unusable (at $n=6$ the mean of
twenty thousand probes was $89\%$ one probe), and validated against the exact tree at $n=4$ to within $0.1\%$
--- puts an unaided exhaustive proof at $n=6$ at some $4.6\cdot10^{22}$ nodes.  That is the estimate that sent
us to a decision procedure; it says nothing about the difficulty of the problem, only about the difficulty of
one way of attacking it.
...
\item The upper bounds for $n=5,6$ rest on one SAT solver.  The instances that a single call decides carry
