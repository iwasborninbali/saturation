...
\date{23 August 2026 --- version 1.2\\ \small\texttt{doi:10.5281/zenodo.22019279}}
...
give certified lower bounds $a(7)\ge73$ and $a(8)\ge93$, and prove $a(p)\ge p^2$ for every prime $p$ by an
...
prohibition itself.  The first solver independently brute-forced all $2\,220\,075$ subsets at $n=3$, obtaining
$16$, and ran a second enumerator with no pruning beyond the prohibition, obtaining $16$ and the count
$10\,960$ at $n=4$.

We are explicit that this does not extend far.  At $n=4$ the same traversal reaches $28$ points but does not
exhaust its tree within an hour, having visited $8.85\cdot10^9$ nodes, so for $n\ge4$ it supplies the lower
bound only; the upper bounds there remain the certified ones.  That is the honest division: for $n\le3$ the
values are established twice over, once through the encoding and once without it, and for $n\ge4$ the encoding
is checked but used.
...
8 & 93 & 1.45 & invariant under the full group of axis permutations (order 6)\\\bottomrule
\end{tabular}\end{center}
