...
\end{lemma}
\begin{proof}
A generic orbit consists either of two classes of type $A$ and two of type $D$, or of two of type $B$ and two of
type $C$ (Lemma~\ref{lem:partners}: $\sigma$ preserves $A$ and $D$ and exchanges $B,C$, while $\tau$ does the opposite), and in the first
case the two $\sigma$-pairs share their centres while the two $\tau$-pairs do not, in the second case the roles are exchanged; the
reflection $(x,y)\mapsto(y,x)$ carries one case to the other, so all generic orbits have the same incidence structure, with rich-line
profile $(4,4,3,3,3,3)$ (verified for every orbit of every $p\le19$: exactly two incidence types occur, one per orbit size).  Likewise the
two classes of an exceptional orbit are $\{\kappa,\nu\kappa\}$ with $\kappa$ fixed by $\sigma$ or by $\tau$, giving profile $(3,3)$.  The
