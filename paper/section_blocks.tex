...
\]
a finite computation for each $K$.  With $K=8$ (the table of $s(\varepsilon)$ was computed twice independently, \texttt{slack/t221/fast\_blocks.py} and
\texttt{slack/sig\_savings.py}, with no discrepancy on the $28$ signatures common to the first tables and none later; two signatures of
length~$8$, of total density $10^{-6}$, are still counted as $0$, which only weakens the bound) the sum is at least
$2778667/5160960$, so
\[
 \alpha(\Pset_{-1})\ \le\ \frac{17865173}{5160960}\,(p-1)+o(p)\ =\ (3.46160\ldots+o(1))\,(p-1).
\]
Since $s(\varepsilon)\le4(k-1)$, the terms with $k>8$ contribute at most $\sum_{k>8}2^{-(k+2)}4(k-1)=0.0352$, so the constant of this
certificate lies in $[3.427,\,3.4616]$; numerically it is $3.4413\ldots$
\end{corollary}
