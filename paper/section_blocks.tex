...
\subsection*{The blocks: runs of consecutive quadratic residues}
...
\]
and the sums are explicit finite computations.  With $K=8$ this gives $\alpha(\Pset_{-1})\le(3.47+o(1))(p-1)$; the terms omitted
($k>K$) satisfy $s\le4(k-1)$, so the full sum lies within $\sum_{k>K}2^{-(k+2)}4(k-1)=0.035$ of the truncation at $K=8$, and numerically
it equals $0.5587$, i.e.\ $\alpha(\Pset_{-1})\le(3.4413+o(1))(p-1)$.
\end{corollary}
