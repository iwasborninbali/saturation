...
and every line carrying at least three points of $\Pset_{-1}$ lies inside a single $B_\varrho$.  Consequently
\begin{equation}\label{eq:blocks}
 \alpha(\Pset_{-1})\ \le\ \sum_{\varrho}\beta(B_\varrho),\qquad
 \beta(B):=\max\{|S|:\ S\subseteq B,\ |S\cap\ell|\le2\ \text{for every line }\ell\},
\end{equation}
and the same sum with $\beta$ replaced by the value of the linear relaxation equals the value of the linear relaxation for
$\Pset_{-1}$ with rows, columns and slope-$(\pm1)$ lines.
\end{theorem}
\begin{proof}
...
block.  Slope $-1$ is symmetric under $R$.  Since the blocks partition $\Pset_{-1}$ and no line meets two of them, a lawful $S$ satisfies
$|S|=\sum_\varrho|S\cap B_\varrho|\le\sum_\varrho\beta(B_\varrho)$, and the constraint matrix of the linear relaxation is block diagonal.
\end{proof}
...
\]
a finite computation for each $K$.  With $K=8$ (the table of $s(\varepsilon)$ is in \texttt{slack/t221/}, computed twice
independently) the sum equals $694193/1290240$, so
\[
 \alpha(\Pset_{-1})\ \le\ \frac{4466767}{1290240}\,(p-1)+o(p)\ =\ (3.46199\ldots+o(1))\,(p-1).
\]
Since $s(\varepsilon)\le4(k-1)$, the terms with $k>8$ contribute at most $\sum_{k>8}2^{-(k+2)}4(k-1)=0.0352$, so the constant of this
certificate lies in $[3.427,\,3.462]$; numerically it is $3.4413\ldots$
\end{corollary}
...
$3.595$ for $L_0=5$, $3.480$ for $L_0=7$ and $3.452$ for $L_0=10$.  The bound~\eqref{eq:blocks} is \emph{below} the value of the linear relaxation with all these
lines ($3.4286$ vs $3.4286$ at $p=113$, $3.3824$ vs $3.3873$ at $p=137$, $3.4343$ vs $3.4545$ at $p=199$, $3.4100$ vs $3.4300$ at $p=401$),
because $\beta$ is an integer maximum inside each block.
(c) The decomposition explains the plateau of Section~\ref{sec:beyond}: no certificate built from rows, columns and slope-$(\pm1)$ lines can
