...
\date{22 August 2026 --- version 1.6}
...
measured value applied to a whole series inflates or deflates every $\chi^2$ that follows.

Even per-point seed spread understates the error, and we can say by how much, because the whole
column was computed twice by implementations sharing no code.  The two disagree by
$1.9$, $2.5$, $1.6$, $0.2$, $3.2$, $3.2$ and $5.5$ times the seed spread at
$n=10,\dots,40$: at $n=40$, by $5.5\%$ where the seeds claim $0.98\%$.  Seeds measure the
variance of one traversal order; everything that differs between implementations --- cell
ordering, selection rule, weight accumulation --- is invisible from inside a single one.  The
honest error bar is the cross-implementation one, and it cannot be obtained by any number of
seeds or descents.  We note that the discrepancy grows with $n$: the further from the exactly
known values, the more confident a single implementation's error bar becomes, and the less it
deserves to be.  All figures below use the mean of the two implementations with the
cross-implementation spread as the error.

This matters beyond bookkeeping.  Run with one implementation's own error bars at $n=40$, every
one of the four forms below is rejected --- $\chi^2$ of $27.6$, $19.3$, $24.8$, $13.1$ on two
degrees of freedom --- and we would have reported that no form fits.  Honest error bars turn that
false certainty back into the correct verdict, which is that the forms cannot be told apart.

The rejection is confined to small $n$.  Taken on tails, three-parameter forms are admissible
from $n\ge15$ onward: $\chi^2=1.2/3$, $0.9/2$, $0.8/1$ for $c+an+bn\ln n$ at $n\ge15,20,25$.
...
across three nested tails, while the purely linear $c+an$ is rejected at $\chi^2=1145/4$,
$332/3$, $19/2$.  The significance of $b$ is $81$, $37$ and $15$ standard deviations on the
...
\midrule
$c+an+b\,n\ln n$ & $0.87$ & constant moves\\
$c+an+d\sqrt n$ & $0.64$ & constant intact\\
$c+an+e\,n/\ln n$ & $0.76$ & constant intact\\
$c+an+f\ln n$ & $0.68$ & constant intact\\
\bottomrule
\end{tabular}
\end{center}
