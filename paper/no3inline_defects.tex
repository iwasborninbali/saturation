...
half-turn (three inequivalent ones); for $n=37$ (two) and $n=39$ (one), the first located configurations of that
...
database as of 2026-08-11 and awaits confirmation by its maintainer.  Configurations, programs and sweep journals are
public.
\end{abstract}
...
$\Dfour$ of the square, and marks the cells for which no configuration is known).
...
family; both database cells were empty), two for $n=37$ and one for $n=39$ ($3$-cycle family; the first located
...
Flammenkamp uses ``defect'' for the shortfall from $2n$ points; to avoid a clash we say \emph{orbit defect}.
...
database, whose rot2 column is complete for $n\le31$ --- and none at $n=15,\dots,25$ and $33$; the corrected counts are
...
37 & 3417 (153) & $\ge2$ & 0 & V2 base complete; C4 base: 3264 non-central sub-classes complete (2 configurations), the 153 central ones in progress\\
39 & 4047 (171) & $\ge1$ & $\ge0$ & C4 base: 300 sub-classes swept (1 configuration); V2 base: 3277 swept (none)\\
41 & 4750 (190) & $\ge0$ & $\ge0$ & V2 base: 4560 non-central sub-classes complete (none), central in progress; C4 base: 3030 swept (none)\\
...
26 & 78 & 1 (complete)\\
28 & 91 & 1 (complete)\\
36 & 153 & $\ge3$: 152 sub-classes exhausted (three of them contain a configuration, two labelled solutions each, i.e.\ one
configuration up to the diagonal reflections); the sub-class $\{1,4\}$ exceeded the one-hour limit and is being rerun\\
\bottomrule
\end{tabular}
...
\cite{FlammenkampDB} are given in the Appendix.  ``First located'' refers to the public files of the database on 2026-08-11
(the database's history page mentions further contributions in preparation at that time), and to a search of the
literature; it awaits the maintainer's confirmation.  For $n=36$ both database cells were empty; for $n=37$ and $39$ the
column \texttt{c} (rct4) was populated, so the configurations below are new representatives of exact half-turn symmetry
outside the rct4 structure, not the first configurations with that stabilizer; the $n=33$ configuration is new but the cell
was not empty.  All seven configurations were checked to be pairwise inequivalent (canonical forms under $\Dfour$).
...
within its own $2p\times2p$ box and unions of two hyperbolae gain only a bounded amount as far as computed; see the companion
note \cite{Companion}.
\end{itemize}

\section{Reproducibility}
The programs, the sub-class lists, the sweep journals and the certified configurations are in the public repository
\url{https://github.com/iwasborninbali/saturation} (immutable tag \texttt{defects-v0.7}; checksums in
...
first agent's machine (the first halves of the lists for $n=39,41,45$) are being added; the $n=33$ and non-central $n=37$
rows are being rerun with the corrected generator on a cloud machine, journals to follow.  All exhaustive statements are
